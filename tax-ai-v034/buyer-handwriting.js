// V0.34 - Handwritten buyer BAN recognition for Taiwan manual invoices.
// Loaded after V0.33 stamp-orientation.js. Wraps the existing scan pipeline.
(function(){
  ...
  panel.innerHTML=`
    <div class="section-title"><div><h2>✍️ 5. 手寫買受人統編強化辨識</h2>
      <div class="muted">手開／三聯式發票：局部裁切 → 去表格線 → 數字專用 OCR → 逐碼辨識 → 統編檢查碼 → 與申報單位統編比對。</div></div>
      ...
      <div>
        ...
        <div class="field" style="margin-top:10px"><label>辨識區域</label>
          <select id="handBuyerPreset">
            <option value="auto">自動比較多個區域</option>
            <option value="upperWide">上方偏右（常見三聯式）</option>
            <option value="upperMid">上方中央</option>
            <option value="midWide">中上方偏右</option>
          </select>
        </div>
  ...
  const presets={
    upperWide:{x:.24,y:.105,w:.72,h:.22,label:'上方偏右'},
    upperMid:{x:.15,y:.145,w:.80,h:.22,label:'上方中央'},
    midWide:{x:.20,y:.22,w:.76,h:.21,label:'中上方偏右'}
  };
  const confusion={
  ...
  function preprocess(src){
    ...
    const avg=sum/Math.max(1,count),thr=Math.max(125,Math.min(210,avg*.88));
    ...
    // Remove long printed horizontal / vertical rules while preserving handwriting strokes.
    for(let y=0;y<c.height;y++){
      let n=0;for(let x=0;x<c.width;x++)n+=dark[y*c.width+x];
      if(n/c.width>.58)for(let yy=Math.max(0,y-1);yy<=Math.min(c.height-1,y+1);yy++)for(let x=0;x<c.width;x++){const i=(yy*c.width+x)*4;d[i]=d[i+1]=d[i+2]=255;dark[yy*c.width+x]=0;}
    }
    for(let x=0;x<c.width;x++){
      let n=0;for(let y=0;y<c.height;y++)n+=dark[y*c.width+x];
      if(n/c.height>.72)for(let xx=Math.max(0,x-1);xx<=Math.min(c.width-1,x+1);xx++)for(let y=0;y<c.height;y++){const i=(y*c.width+xx)*4;d[i]=d[i+1]=d[i+2]=255;dark[y*c.width+xx]=0;}
    }
    ctx.putImageData(im,0,0);return c;
  }
  function digitString(s){return String(s||'').replace(/[^0-9]/g,'');}
  function cropInk(src){
    ...
    if(maxX<0||maxY<0)return src;const padX=Math.round((maxX-minX+1)*.04),padY=Math.round((maxY-minY+1)*.12);minX=Math.max(0,minX-padX);maxX=Math.min(src.width-1,maxX+padX);minY=Math.max(0,minY-padY);maxY=Math.min(src.height-1,maxY+padY);
  ...
  async function recognizeLine(worker,canvas){
    ...
    const r=await worker.recognize(canvas);return {digits:digitString(r.data.text).slice(0,12),confidence:Number(r.data.confidence)||0,text:r.data.text||''};
  }
  function splitEight(src){
    const ink=cropInk(src),out=[];const w=ink.width/8;
    for(let i=0;i<8;i++){
      const x=Math.max(0,Math.floor(i*w)-2),x2=Math.min(ink.width,Math.ceil((i+1)*w)+2);const c=document.createElement('canvas');c.width=Math.max(1,(x2-x)*2);c.height=ink.height*2;
  ...
  async function recognizeChars(worker,src){
    ...
    return {digits:digits.join(''),digitArray:digits,confs};
  }
  function candidateScore(digits,conf){
    const company=fixDigits(companyBan.value),d=digitString(digits);let s=Math.min(75,Number(conf)||0);
    if(d.length===8)s+=35;if(d.length===8&&validBan(d))s+=70;if(company&&d===company)s+=85;return s;
  }
  function beamValid(primary,confs){
    ...
    for(let i=0;i<8;i++){
      ...
      next.sort((a,b)=>a.pen-b.pen);beams=next.slice(0,300);
    }
    const company=fixDigits(companyBan.value);const valid=beams.filter(x=>validBan(x.s));if(!valid.length)return null;
    valid.sort((a,b)=>{const am=company&&a.s===company?-1000:0,bm=company&&b.s===company?-1000:0;return (a.pen+am)-(b.pen+bm)});return valid[0];
  }
  function renderDigits(digits,confs){
    const arr=Array.from({length:8},(_,i)=>digits?.[i]||'');digitBoxes.innerHTML=arr.map((d,i)=>`<div class="digitbox"><input inputmode="numeric" maxlength="1" data-i="${i}" value="${d}"><small>${confs?.[i]??'—'}%</small></div>`).join('');
    digitBoxes.querySelectorAll('input').forEach(inp=>inp.oninput=()=>{inp.value=inp.value.replace(/\D/g,'').slice(0,1);refreshManual();});
  }
  function currentDigits(){return [...digitBoxes.querySelectorAll('input')].map(x=>x.value).join('');}
  function refreshManual(){const d=currentDigits();candidateEl.textContent=d||'—';checksumEl.textContent=d.length===8?(validBan(d)?'✅ 通過':'❌ 未通過'):'尚未 8 碼';const company=fixDigits(companyBan.value);companyMatchEl.textContent=!company?'未設定申報單位':d===company?'✅ 等於申報單位統編':'不相同';applyBtn.disabled=d.length!==8||!validBan(d);lastResult={...(lastResult||{}),digits:d,valid:d.length===8&&validBan(d),companyMatch:!!company&&d===company};}
  async function runBuyerOCR(corrected,forcedPreset){
    if(!corrected)return null;badge.textContent='辨識中';hbStatus.className='info';hbStatus.textContent='正在定位並辨識手寫買受人統編…';applyBtn.disabled=true;
    const keys=forcedPreset&&forcedPreset!=='auto'?[forcedPreset]:Object.keys(presets),worker=await Tesseract.createWorker('eng',1,{logger:m=>{if(m.progress!=null)hbStatus.textContent=`手寫數字 OCR ${Math.round(m.progress*100)}%`;}}),results=[];
    try{
      for(const k of keys){const rawCrop=crop(corrected,presets[k],2.5),pp=preprocess(rawCrop),line=await recognizeLine(worker,pp);results.push({key:k,rawCrop,pp,...line,score:candidateScore(line.digits,line.confidence)});}
      results.sort((a,b)=>b.score-a.score);let best=results[0];if(!best)return null;
      let charRes=null,chosen=best.digits.slice(0,8);
      if(chosen.length!==8||!validBan(chosen)||best.confidence<80){charRes=await recognizeChars(worker,best.pp);const primary=charRes.digits.length===8?charRes.digits:chosen;const confs=charRes.confs||Array(8).fill(Math.round(best.confidence));const alt=beamValid(primary,confs);if(alt)chosen=alt.s;else if(primary.length===8)chosen=primary;}
      ...
      const confs=charRes?.confs||Array(8).fill(Math.round(best.confidence));renderDigits(chosen,confs);lastResult={digits:chosen,valid,companyMatch,confidence:Math.round(best.confidence),preset:best.key,score:best.score,confs};refreshManual();
      badge.textContent=valid?(companyMatch?'高可信':'合法候選'):'需人工確認';hbStatus.className=valid?'ok':'warn';hbStatus.textContent=valid?(companyMatch?`✓ 手寫買受人統編 ${chosen}，檢查碼通過且等於申報單位。`:`✓ 辨識候選 ${chosen} 通過統編檢查碼，但與申報單位不同，請人工確認。`):`⚠ 尚未取得通過檢查碼的 8 碼統編，請逐碼修正或調整辨識區域。`;
      if(valid){addCandidate('buyer',chosen,companyMatch?'手寫買受人統編：逐碼＋申報單位比對':'手寫買受人統編：逐碼＋檢查碼',companyMatch?116:101);chooseFields();validateRecognition();renderSourceDetail();}
      return lastResult;
  ...
  applyBtn.onclick=()=>{const d=currentDigits();if(d.length!==8||!validBan(d)){hbStatus.className='warn';hbStatus.textContent='⚠ 請先修正為通過統編檢查碼的 8 碼數字。';return;}addCandidate('buyer',d,'人工確認：手寫買受人統編',125);buyer.value=d;setSource('buyer','人工確認：手寫買受人統編');const v=validateRecognition();renderSourceDetail();hbStatus.className='ok';hbStatus.textContent=`✓ 已人工套用買受人統編 ${d}；${v.reason}`;};
  rerun.onclick=async()=>{const c=lastCorrected||canvasFromCorrectedPreview();if(!c){hbStatus.className='warn';hbStatus.textContent='請先上傳並完成一次發票辨識。';return;}rerun.disabled=true;try{await runBuyerOCR(c,preset.value);}finally{rerun.disabled=false;}};

  const baseScan=scanBtn.onclick;
  scanBtn.onclick=async function(){
    await baseScan.call(scanBtn);
    const type=invoiceType?.value||'auto';const isHand=type==='hand'||(type==='auto'&&state.qr.length===0);
    ...
    scanBtn.disabled=true;try{status.textContent='✍️ 加強辨識手寫買受人統編…';await runBuyerOCR(lastCorrected,preset.value);const v=validateRecognition();renderSourceDetail();status.textContent=`辨識完成：${v.reason}，請人工核對。`;scanSummary.textContent+=' 已加跑手寫買受人統編專用 OCR。';}catch(e){hbStatus.className='warn';hbStatus.textContent='手寫買受人統編辨識失敗：'+(e.message||e);}finally{scanBtn.disabled=false;}
  };
})();
