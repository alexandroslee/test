...
規則：
1. 「銷售額」就是 sales_amount，也就是未稅金額。
2. 「稅額／營業稅」就是 tax_amount。
3. 「總計／總額／合計」就是 total_amount，也就是含稅總額。
4. 若票面有「應稅／零稅率／免稅」三欄，必須看 V、✓、勾、黑點等選取符號相對於三個標籤的位置。
5. V 在「應稅」旁或其底線位置，tax_category="應稅"；在「零稅率」旁則為「零稅率」；在「免稅」旁則為「免稅」。看不清楚回「待確認」。
6. 不可只因稅額 > 0 就宣稱看到了勾選；tax_category_evidence 必須描述真正看到的位置。
7. 台灣三聯式發票左上買受人區 8 格統編 = buyer_tax_id；右下發票章統編 = seller_tax_id，不可交換。
8. invoice_number 是 2 個英文字母 + 8 個數字，不可把統編當發票號碼。
9. 看不清楚就回空值或待確認，不可硬猜。
""".strip()

TAX_CATEGORY_PROMPT = r"""
這張圖片是台灣統一發票的「課稅別」小區塊，通常同時看到「應稅」「零稅率」「免稅」與 V／✓／勾／黑點／圈選標記。
你的唯一工作是判斷哪一欄被選取，只看圖片，不使用稅額或公式猜答案。
- 標記位於「應稅」旁或其底線位置：category="應稅"
- 標記位於「零稅率」旁：category="零稅率"
- 標記位於「免稅」旁：category="免稅"
- 標記不清楚：category="待確認"
evidence 必須描述標記實際位置。
只輸出 JSON：
...
BUYER_BAN_PROMPT = r"""
The image contains exactly eight adjacent buyer-tax-ID cells, one visible decimal digit per cell.
Read strictly LEFT TO RIGHT. Ignore borders, stamps and noise.
Never use a checksum to guess, repair or substitute digits.
If any cell cannot be read, buyer_tax_id must be null.
Return ONLY JSON:
...
def _decode_image(value: Any) -> Image.Image:
    ...
    if isinstance(value, dict):
        ...
        if path: return Image.open(path).convert("RGB")
...
def _generate(image: Image.Image, prompt: str, max_new_tokens: int):
    ...
    return final,{"input_keys":list(inputs.keys()),"pixel_values":list(inputs["pixel_values"].shape),"visual_token_budget":int(processor.image_processor.max_soft_tokens)}
...
def invoice_api(image_data: str) -> dict:
    ...
    sales=_money(obj.get("sales_amount")); tax=_money(obj.get("tax_amount")); total=_money(obj.get("total_amount")); category=_clean_tax_category(obj.get("tax_category")); warnings=[]
    if sales is not None and tax is not None and total is not None and sales+tax!=total: warnings.append(f"金額一致性警告：{sales}+{tax}!={total}；保留視覺辨識值。")
    if category in {"零稅率","免稅"} and tax is not None and tax>0: warnings.append(f"課稅別衝突：Vision={category}，但稅額={tax}>0；請人工核對。")
    data={"invoice_type":str(obj.get("invoice_type") or "其他"),"invoice_number":_clean_invoice_no(obj.get("invoice_number")),"invoice_date":str(obj.get("invoice_date") or ""),"seller_tax_id":_clean_tax_id(obj.get("seller_tax_id")),"buyer_tax_id":_clean_tax_id(obj.get("buyer_tax_id")),"seller_name":str(obj.get("seller_name") or ""),"sales_amount":sales,"tax_amount":tax,"total_amount":total,"tax_category":category,"tax_category_source":str(obj.get("tax_category_source") or "無法辨識"),"tax_category_evidence":str(obj.get("tax_category_evidence") or "")}
    try: confidence=max(0.0,min(1.0,float(obj.get("confidence") or 0)))
    except Exception: confidence=0.0
    return {"backend_version":BACKEND_VERSION,"release_id":RELEASE_ID,"count":1,"results":[{"data":data,"confidence":confidence,"source":"hf-zerogpu-gemma4-e4b-v152","raw_text":raw,"warnings":warnings,"elapsed_ms":int((time.time()-started)*1000),"vision_debug":vision_debug}],"model":MODEL_ID,"checksum_used":False,"tax_category_supported":True,"dedicated_tax_category_api":True}


@spaces.GPU(duration=35)
def tax_category_api(image_data: str) -> dict:
    started=time.time(); image=_decode_image(image_data); raw,vision_debug=_generate(image,TAX_CATEGORY_PROMPT,180); obj=_extract_json(raw); category=_clean_tax_category(obj.get("category")); evidence=str(obj.get("evidence") or "").strip()
    try: confidence=max(0.0,min(1.0,float(obj.get("confidence") or 0)))
    except Exception: confidence=0.0
    return {"backend_version":BACKEND_VERSION,"release_id":RELEASE_ID,"category":category,"evidence":evidence,"confidence":confidence,"source":"Gemma 4 E4B：課稅別專用裁切辨識","raw":raw,"vision_debug":vision_debug,"elapsed_ms":int((time.time()-started)*1000)}


@spaces.GPU(duration=60)
def buyer_ban_api(image_data: str) -> dict:
    started=time.time(); image=_decode_image(image_data); raw,vision_debug=_generate(image,BUYER_BAN_PROMPT,160); obj=_extract_json(raw); ban=_clean_tax_id(obj.get("buyer_tax_id")) if obj.get("buyer_tax_id") is not None else ""; digits=obj.get("digits") if isinstance(obj.get("digits"),list) else (list(ban) if ban else []); digits=[re.sub(r"\D","",str(x))[:1] for x in digits[:8]]
    if len(digits)!=8 or any(len(x)!=1 for x in digits): digits,ban=[],""
    elif not ban:
        joined="".join(digits); ban=joined if len(joined)==8 else ""
    try: confidence=max(0.0,min(1.0,float(obj.get("confidence") or 0)))
    except Exception: confidence=0.0
    return {"backend_version":BACKEND_VERSION,"release_id":RELEASE_ID,"buyer_tax_id":ban or None,"digits":digits,"confidence":confidence,"model":MODEL_ID,"checksum_used":False,"raw":raw,"vision_debug":vision_debug,"elapsed_ms":int((time.time()-started)*1000)}


def health_api() -> dict:
    return {"status":"ok","backend":"huggingface-zerogpu","backend_version":BACKEND_VERSION,"release_id":RELEASE_ID,"model":MODEL_ID,"gpu_mode":"ZeroGPU","visual_token_budget":int(processor.image_processor.max_soft_tokens),"tax_category_supported":True,"dedicated_tax_category_api":True,"tax_category_values":["應稅","零稅率","免稅","待確認"],"checksum_used":False}
...
with gr.Blocks(title="Tax AI ZeroGPU V1.5.2 — Gemma 4 E4B") as demo:
    gr.Markdown("# 🧾 Tax AI ZeroGPU V1.5.2 — Gemma 4 E4B\n電子發票 QR＋課稅別專用裁切辨識。")
    with gr.Tab("整張發票"):
    ...
    with gr.Tab("課稅別"):
        img_tax=gr.Image(type="pil",label="應稅／零稅率／免稅裁切"); out_tax=gr.JSON(label="課稅別結果"); gr.Button("Gemma 課稅別辨識").click(ui_tax_category,inputs=img_tax,outputs=out_tax)
    with gr.Tab("買受人8格"):
    ...
    tax_in=gr.Textbox(visible=False); tax_out=gr.JSON(visible=False); gr.Button(visible=False).click(tax_category_api,inputs=tax_in,outputs=tax_out,api_name="tax_category_api")
