(function(){
  ...
  const VERSION='1.5.9';
  const BUILD='20260831-v159-nemotron-parse-r4';
  const MODEL='nvidia/NVIDIA-Nemotron-Parse-2.0';
  const RELEASE='tax-ai-1.5.9-nemotron-parse-20260831-r4';
  ...
  function ensureCard(){
    ...
    card.innerHTML=`<h2>🟢 NVIDIA Nemotron Parse 2.0｜V1.5.9 主文件模型</h2>
      ...
      <div id="nemotron159Status" class="info" style="margin-top:10px">等待 V1.5.9 r4 後端連線；本地 OCR 可先工作，Nemotron 結果回來後再升級主值。</div>
  ...
  async function verify(showResult=false){
    ...
    try{
      ...
      if(showResult||verified)status(verified?`✓ Nemotron Backend V${r.backend_version} r4 已驗證｜${r.model}`:`⚠ Backend Release Contract 不符：${r?.backend_version||'?'} / ${r?.release_id||'?'} / ${r?.model||'?'}`,verified?'ok':'warn');
      return verified;
  ...
  function applyResult(resp){
    if(String(resp?.backend_version||'')!==VERSION||String(resp?.release_id||'')!==RELEASE||String(resp?.model||'')!==MODEL)throw new Error('Nemotron 回傳的 Release Contract 不符 V1.5.9 r4');
    ...
    window.__taxAiNemotronResult159=resp;status(`✓ Nemotron Parse 2.0 r4 完成：${first.blocks?.length||0} 個文件區塊；空間規則已對賬並回寫。綜合信心 ${Math.round(conf*100)}%。`,'ok');return first;
  }
  async function runPrimary({manual=false}={}){
    if(running)return null;running=true;ensureCard();
    try{
      if(!verified&&!(await verify(manual))){if(!manual)status('ℹ Nemotron Backend 尚未通過 V1.5.9 r4 驗證；保留本地 provisional 結果，不阻塞操作。','info');return null}
  ...
  function patchScan(){
    ...
    scan.onclick=async function(...args){const r=typeof old==='function'?await old.apply(this,args):undefined;status('✓ 本地初步辨識完成；Nemotron Parse 2.0 r4 正在背景解析，畫面可繼續操作。','info');setTimeout(()=>runPrimary({manual:false}),20);return r};
  }
