(function(){
  ...
  function parseTaiwanQr(raw){
    ...
    return {
      raw:q,
      track:inv.slice(0,2),number:inv.slice(2),date:`${y}-${m}-${d}`,
      seller,buyer:/^0{8}$/.test(buyer)?'':buyer,
      net,gross,tax:gross-net
    };
  }
  ...
  function ensurePanel(){
    ...
    p=document.createElement('div');p.id='qrAuthority149';p.className='ok';p.style.marginTop='10px';
    p.innerHTML='<b>📱 電子發票 QR 權威資料</b><div id="qrAuthority149Body" style="margin-top:6px">等待標準電子發票 QR。</div>';
  ...
  function render(p,sourceLabel){
    ...
    body.innerHTML=`<b>${p.track}-${p.number}</b>　日期 ${p.date}<br>賣方統編 <b>${p.seller}</b>　買方統編 <b>${p.buyer||'未登載'}</b><br><span style="font-size:1.08em">未稅／銷售額 <b>${p.net.toLocaleString('zh-TW')}</b>　稅額 <b>${p.tax.toLocaleString('zh-TW')}</b>　含稅總額 <b>${p.gross.toLocaleString('zh-TW')}</b></span><br><small>${sourceLabel}｜QR 為電子發票權威來源，Gemma 僅作交叉驗證。</small>`;
  }
  ...
  function fromState(){
    try{
      for(const q of (state?.qr||[])){const p=parseTaiwanQr(q);if(p)return apply(p,'QR Code（原始資料直接解析）')}
    }catch{}
    return null;
  }
  ...
  function scanCanvas(c){
    ...
    const regions=[
      [0,0,W,H],[0,0,Math.ceil(W*.58),H],[0,0,W,Math.ceil(H*.58)],
      [0,Math.floor(H*.35),Math.ceil(W*.6),Math.ceil(H*.65)],
      [0,0,Math.ceil(W*.55),Math.ceil(H*.55)],
      [Math.floor(W*.45),0,Math.ceil(W*.55),Math.ceil(H*.55)],
      [0,Math.floor(H*.45),Math.ceil(W*.55),Math.ceil(H*.55)],
      [Math.floor(W*.45),Math.floor(H*.45),Math.ceil(W*.55),Math.ceil(H*.55)]
    ];
    for(const [x,y,w0,h0] of regions){
      const w=Math.min(w0,W-x),h=Math.min(h0,H-y);if(w<60||h<60)continue;
      try{
        const im=ctx.getImageData(x,y,w,h);const r=jsQR(im.data,w,h,{inversionAttempts:'attemptBoth'});
        if(r?.data){const p=parseTaiwanQr(r.data);if(p)return {p,raw:r.data}}
      }catch{}
    }
    return null;
  }
  ...
  function fromPreview(){
    ...
    const scale=Math.min(1,1600/Math.max(W,H));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(W*scale));c.height=Math.max(1,Math.round(H*scale));
  ...
  function payloadFor(p){
    return {
      count:1,
      results:[{
        data:{invoice_type:'電子發票',invoice_number:`${p.track}-${p.number}`,invoice_date:p.date,seller_tax_id:p.seller,buyer_tax_id:p.buyer,seller_name:'',sales_amount:p.net,tax_amount:p.tax,total_amount:p.gross},
        confidence:1,
        source:'taiwan-einvoice-qr-authority',
        raw_text:`QR authority: ${p.track}-${p.number}; sales=${p.net}; tax=${p.tax}; total=${p.gross}`,
        warnings:[],elapsed_ms:0,amount_semantics:{sales_amount:'QR 銷售額＝未稅',tax_amount:'QR 總額－銷售額',total_amount:'QR 含稅總額',printed_values_preferred:true}
      }],
      model:'Taiwan e-Invoice QR',checksum_used:false,qr_authoritative:true
    };
  }
  ...
  window.fetch=async function(input,init){
    ...
    if(submit&&method==='POST'){
      const p=resolveQr();
      if(p){
        const id='qr149-'+Date.now().toString(36)+Math.random().toString(36).slice(2,7);
        synthetic.set(id,payloadFor(p));
        return new Response(JSON.stringify({event_id:id}),{status:200,headers:{'content-type':'application/json','x-tax-ai-source':'qr-authority-v149'}});
      }
    }
