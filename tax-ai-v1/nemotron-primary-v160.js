(function(){
  if(window.__taxAiNemotron160)return;
  window.__taxAiNemotron160=true;
  const VERSION='1.6.0';
  const BUILD='20260831-v160-nemotron-parse-r1';
  const MODEL='nvidia/NVIDIA-Nemotron-Parse-2.0';
  const RELEASE='tax-ai-1.6.0-nemotron-parse-20260831-r1';
  ...
  function ensureCard(){
    let card=$('nemotron160Card');if(card)return card;
    card=document.createElement('div');card.id='nemotron160Card';card.className='card';card.style.marginTop='16px';
    card.innerHTML=`<h2>🟢 NVIDIA Nemotron Parse 2.0｜V1.6.0 主文件模型</h2>
      ...
      <input id="nemotron160Url" value="${DEFAULT_SPACE}" style="width:100%;box-sizing:border-box;padding:10px;border:1px solid #d7dfeb;border-radius:10px;margin-top:6px">
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
        <button id="nemotron160Health" class="btn ghost">🔌 驗證 Nemotron Backend</button>
        <button id="nemotron160Run" class="btn">🟢 Nemotron Parse 2.0 主辨識</button>
      </div>
      <div id="nemotron160Status" class="info" style="margin-top:10px">等待 V1.6.0 r1 後端連線；本地 OCR 可先工作，Nemotron 結果回來後再升級主值。</div>
      <div id="nemotron160Evidence" class="info" style="margin-top:10px;display:none"></div>`;
    ...
    $('nemotron160Health').onclick=()=>verify(true);$('nemotron160Run').onclick=()=>runPrimary({manual:true});
    return card;
  }
  function status(text,cls='info'){ensureCard();const e=$('nemotron160Status');if(e){e.className=cls;e.textContent=text}}
  async function mod(){if(!modulePromise)modulePromise=import(CLIENT_URL);return modulePromise}
  async function connect(){
    if(clientPromise)return clientPromise;
    const url=String($('nemotron160Url')?.value||DEFAULT_SPACE).trim()||DEFAULT_SPACE;
  ...
  async function verify(showResult=false){
    ...
    try{
      ...
      window.__taxAiNemotronHealth160=r;
      if(showResult||verified)status(verified?`✓ Nemotron Backend V${r.backend_version} r1 已驗證｜${r.model}`:`⚠ Backend Release Contract 不符：${r?.backend_version||'?'} / ${r?.release_id||'?'} / ${r?.model||'?'}`,verified?'ok':'warn');
      return verified;
  ...
  function write(id,v,source,promote=false){
    ...
    el.value=value;el.dataset.aiSource=source;el.dataset.nemotron160='1';
  ...
  function renderEvidence(first){
    const e=$('nemotron160Evidence');if(!e)return;const ev=first?.evidence||{},d=first?.data||{},a=ev.amounts||{},tc=ev.tax_category||{};
  ...
  function applyResult(resp){
    if(String(resp?.backend_version||'')!==VERSION||String(resp?.release_id||'')!==RELEASE||String(resp?.model||'')!==MODEL)throw new Error('Nemotron 回傳的 Release Contract 不符 V1.6.0 r1');
    ...
    window.__taxAiNemotronResult160=resp;status(`✓ Nemotron Parse 2.0 r1 完成：${first.blocks?.length||0} 個文件區塊；空間規則已對賬並回寫。綜合信心 ${Math.round(conf*100)}%。`,'ok');return first;
  }
  async function runPrimary({manual=false}={}){
    if(running)return null;running=true;ensureCard();
    try{
      if(!verified&&!(await verify(manual))){if(!manual)status('ℹ Nemotron Backend 尚未通過 V1.6.0 r1 驗證；保留本地 provisional 結果，不阻塞操作。','info');return null}
  ...
  function patchScan(){
    const scan=$('scan');if(!scan||scan.dataset.nemotron160==='1')return;scan.dataset.nemotron160='1';const old=scan.onclick;
    scan.onclick=async function(...args){const r=typeof old==='function'?await old.apply(this,args):undefined;status('✓ 本地初步辨識完成；Nemotron Parse 2.0 r1 正在背景解析，畫面可繼續操作。','info');setTimeout(()=>runPrimary({manual:false}),20);return r};
  }
  function patch(){ensureCard();patchScan()}
  patch();setTimeout(patch,200);setTimeout(patch,900);setTimeout(()=>verify(false),1600);
  window.__taxAiNemotron160Api={VERSION,BUILD,MODEL,RELEASE,DEFAULT_SPACE,verify,runPrimary,applyResult,validBan};
  console.info('[TaxAI] V1.6.0 Nemotron Parse primary pipeline active',BUILD);
})();